...
void main() {
  ...
  vec2 mousePull = (uMouse - 0.5) * 0.012 * uIntensity;
  vec2 refractOffset = normal * (0.006 + uRefraction * 0.028) * uIntensity;
  refractOffset += centered * edge * uRefraction * 0.012;
  refractOffset += mousePull * (0.35 + edge);
  refractOffset += (n - 0.5) * uDistortion * 0.025;

  vec2 distortedUv = clamp(uv + refractOffset, 0.002, 0.998);
  float blurPx = mix(0.8, 10.0, clamp(uBlur / 28.0, 0.0, 1.0));
  ...
  base.r = mix(base.r, red, 0.34);
  base.b = mix(base.b, blue, 0.34);

  vec3 color = saturateColor(base.rgb, uSaturation) * uBrightness;
  color = mix(color, vec3(0.92, 0.98, 1.0), 0.10 * uIntensity);
  ...
  float inner = smoothstep(0.72, 0.0, length(centered)) * 0.045;

  color += vec3(0.72, 0.92, 1.0) * fresnel * 0.42;
  color += vec3(1.0, 1.0, 0.96) * (spec + sweep + inner);

  float alpha = roundedRectAlpha(uv, uBorderRadius);
  float opacity = clamp(uOpacity + fresnel * 0.18 + spec * 0.08, 0.0, 0.96);
